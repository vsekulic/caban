...
def plot_lt_spatial_responses(PLOTS_DIR, LT1_group, LT2_group, mouse_groups, session_str, mapping=None):
    ...
    Additionally computes PV correlation matrix (LT1 bins x LT2 bins) for each mouse:
      pv_corr_LT1_LT2[mouse] = {
          "pv_corr": (n_bins, n_bins) array,
          "bin_centers_LT1": (n_bins,) array,
          "bin_centers_LT2": (n_bins,) array,
          "sorted_LT1": list of LT1 cell ids in row order,
          "sorted_LT2": list of LT2 cell ids in the same row order
      }

    Returns pv_corr_LT1_LT2.
    '''
    if mapping is None:
    ...
    # -------------------------
    # Output dict (requested)
    # -------------------------
    pv_corr_LT1_LT2 = {}

    # -------------------------
    # Helper: PV correlation matrix
    # -------------------------
    def _pv_corr_matrix(tuning1_cells_by_bin, tuning2_cells_by_bin, eps=1e-12):
        """
        tuning1_cells_by_bin: (n_cells, n_bins1)
        tuning2_cells_by_bin: (n_cells, n_bins2)
        Returns: (n_bins1, n_bins2) Pearson r across cells for each bin pair.
        """
        t1 = np.asarray(tuning1_cells_by_bin, float)
        t2 = np.asarray(tuning2_cells_by_bin, float)

        n_cells1, n_bins1 = t1.shape
        n_cells2, n_bins2 = t2.shape
        if n_cells1 != n_cells2:
            raise ValueError(f"PV corr requires same #cells. Got {n_cells1} vs {n_cells2}.")

        out = np.full((n_bins1, n_bins2), np.nan, dtype=float)
        ...
        for i in range(n_bins1):
            ...
            for j in range(n_bins2):
                ...
                if denom < eps:
                    continue
                out[i, j] = float(np.sum(a*b) / denom)
    ...
    for mouse, group in mouse_groups.items():
        ...
        # -------------------------
        # Build LT1 tuning (rows sorted by sorted_LT1)
        # -------------------------
        S_LT1 = np.asarray(LT1.S)            # (num_cells, time)
        ...
        # -------------------------
        # Build LT2 tuning in SAME LT1-sorted order
        # -------------------------
        S_LT2 = np.asarray(LT2.S)
        ...
        # Ensure same number of rows (in case partial mapping)
        n_rows = min(tuning1_plot.shape[0], tuning2_plot.shape[0])
        tuning1_plot = tuning1_plot[:n_rows, :]
        tuning2_plot = tuning2_plot[:n_rows, :]
        tuning1_for_pv = tuning1[:n_rows, :]   # IMPORTANT: use pre-display normalization for PV corr
        tuning2_for_pv = tuning2[:n_rows, :]

        # -------------------------
        # PV correlation (store per mouse)
        # -------------------------
        pv_corr = _pv_corr_matrix(tuning1_for_pv, tuning2_for_pv)
        pv_corr_LT1_LT2[mouse] = {
            "pv_corr": pv_corr,
            "bin_centers_LT1": np.asarray(bin_centers1, float),
            "bin_centers_LT2": np.asarray(bin_centers2, float),
            "sorted_LT1": list(sorted_LT1),
            "sorted_LT2": list(sorted_LT2),
        }

        # -------------------------
        # Two-panel plot (no colorbar; NaNs black)
        # -------------------------
        ...
        # -------------------------
        # Optional sanity plots (unchanged)
        # -------------------------
...
# Usage after
plt.imshow(pv_corr_LT1_LT2['G21']['pv_corr'])
avg_diag_corr = np.diag(pv_corr_LT1_LT2['G05']['pv_corr']).mean()

[k for k, v in mouse_groups.items() if v == "mCherry"]
...
# compute avg diagonal correlation using nanmean
avg_diag = {
    mouse: np.nanmean(np.diag(pv_corr_LT1_LT2[mouse]['pv_corr']))
    for mouse in mouse_groups
}

df = pd.DataFrame({
    'mouse': list(avg_diag.keys()),
    'group': [mouse_groups[m] for m in avg_diag],
    'avg_diag_corr': list(avg_diag.values())
})

df_clean = df.dropna(subset=['avg_diag_corr'])
...
for g in df_clean['group'].unique():
    x = list(df_clean['group'].unique()).index(g) + 1
    vals = df_clean[df_clean['group'] == g]
...
plt.title('PV correlation by group')
plt.suptitle('')
plt.ylabel('Average diagonal PV correlation')
plt.tight_layout()
plt.show()

groups = df_clean.groupby('group')['avg_diag_corr']
anova = f_oneway(*[groups.get_group(g).values for g in groups.groups])

print("ANOVA F =", anova.statistic)
print("ANOVA p =", anova.pvalue)

tukey = pairwise_tukeyhsd(
    endog=df_clean['avg_diag_corr'],
