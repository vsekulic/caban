...
class CrossRegMapping:
    ...
    def __init__(self, mouse, dpath_mappings, crossreg_type=1, groups_mappings=None, savepath=''):
        ...
        # Build session_list and session_suffixes dynamically based on number of groups
        n_groups = len(self.groups)
        self.session_suffixes = {0: ''}
        for _i in range(1, n_groups):
            self.session_suffixes[_i] = f'.{_i}'
        self.session_list = ['session'] + [f'session.{_i}' for _i in range(1, n_groups)]

        if crossreg_type == 1:
            # LT1, LT2, TFC_cond — use explicit groups_mappings if provided,
            # otherwise fall back to chronological sort order.
            assert len(self.groups) == 3 # should have 3 unique strings for each of LT1, LT2, TFC_cond
            self.mappings_labels = dict()
            if self.groups_mappings:
                for label in ['LT1', 'LT2', 'TFC_cond']:
        ...
        # TFC_cond/TFC_cond; Test_B/Test_B; Test_B_1wk/Test_B_1wk; Test_A/Test_A; Test_A_1wk/Test_A_1wk
        if crossreg_type == 6:
            self.mappings_labels = dict()
            all_labels = ['TFC_cond', 'Test_A', 'Test_A_1wk', 'Test_B', 'Test_B_1wk']
            for label in all_labels:
                if label in self.groups_mappings:
                    self.mappings_labels[label] = self.groups_mappings[label]

        # TFC_cond; Test_A; Test_A_1wk  (counterpart to crossreg4 for Test B)
        if crossreg_type == 7:
            self.mappings_labels = dict()
            if mouse == 'G15':
                labels = ['TFC_cond', 'Test_A']
            else:
                labels = ['TFC_cond', 'Test_A', 'Test_A_1wk']

            for label in labels:
                self.mappings_labels[label] = self.groups_mappings[label]

        # Resolve mappings_labels values against self.groups to handle timestamp
        # mismatches (e.g. '18_26_30' vs '918_26_30' for G17 TFC_cond).
        if hasattr(self, 'mappings_labels'):
            for label, val in self.mappings_labels.items():
                if val not in self.groups:
                    matches = [g for g in self.groups if g.endswith(val) or val.endswith(g)]
                    if len(matches) == 1:
                        self.mappings_labels[label] = matches[0]

        return

    def get_cells_for_pair(self, *labels):
        """
        Return DataFrame rows for ALL cells registered in every one of
        *labels*, regardless of which other sessions they also appear in.

        Accepts 2+ label strings.  Example::

            cr.get_cells_for_pair('TFC_cond', 'Test_A')          # pair
            cr.get_cells_for_pair('TFC_cond', 'Test_A', 'Test_A_1wk')  # trio

        This is broader than get_mappings_cells('A+B+C'), which only returns
        cells whose *exact* group is {A, B, C}.
        """
        for lab in labels:
            if lab not in self.mappings_labels:
                raise KeyError(f"Label '{lab}' not in mappings_labels")

        ts_needed = [self.mappings_labels[lab] for lab in labels]

        all_rows = []
        for group_str, row_indices in self.mappings.items():
            group_tuple = ast.literal_eval(group_str)
            if all(ts in group_tuple for ts in ts_needed):
                all_rows.extend(row_indices)

        if not all_rows:
            raise KeyError(f"No cells registered in all of {labels}")

        return self.mappings_df.iloc[all_rows]
    ...
    def get_mappings_for_groups(self, mapping_type):
        ...
        for label in groups:
            if label not in self.mappings_labels:
                _wk = (self.mouse, self.crossreg_type, mapping_type, label, "key")
                if _wk not in CrossRegMapping._warned_mappings:
                    CrossRegMapping._warned_mappings.add(_wk)
                    print(f"  [DEBUG get_mappings_for_groups] KeyError incoming!")
                    print(f"    mouse={self.mouse}, crossreg_type={self.crossreg_type}")
                    print(f"    mapping_type='{mapping_type}', label='{label}'")
                    print(f"    mappings_labels keys={list(self.mappings_labels.keys())}")
                    print(f"    groups={self.groups}")
            group_str = self.mappings_labels[label]
            if group_str not in self.groups:
                _wv = (self.mouse, self.crossreg_type, mapping_type, label, "val")
                if _wv not in CrossRegMapping._warned_mappings:
                    CrossRegMapping._warned_mappings.add(_wv)
                    print(f"  [DEBUG get_mappings_for_groups] ValueError incoming!")
                    print(f"    mouse={self.mouse}, crossreg_type={self.crossreg_type}")
                    print(f"    mapping_type='{mapping_type}', label='{label}', group_str='{group_str}'")
                    print(f"    groups={self.groups}")
...
class BehaviourSession:
    ...
    def __init__(self, mouse, dpath, session_bounds=[], plot_sample_cell=False, data_dir='', session_group='session', crossreg='', savepath='', session_type='Behaviour',
        ...
        self.behaviour_type = behaviour_type # movement, immobility
        self.behaviour_condition = behaviour_condition # like hM3D, hM4D
        ...
        self.pyr_percentile_cutoff = pyr_percentile_cutoff # for firing rate cutoff of pyr vs interneurons.
        ...
        self.saver_YrA = Saver(parent_path=savepath, subdirs=['YrA'], prefix='{}_{}_YrA'.format(saver_prefix, mouse))
        self.saver_A = Saver(parent_path=savepath, subdirs=['A_matrix'], prefix='{}_{}_A'.format(saver_prefix, mouse))
        
        # Only create centerline saver for LinearTrackSession
        if type(self).__name__ == 'LinearTrackSession':
            self.saver_LT_centerline = Saver(parent_path=savepath, subdirs=['LT_centerline'], prefix='{}_{}_centerline'.format(mouse, session_type))
    ...
    def get_CS_matrices(self):
        ...
        if self.YrA_full is not None:
            self.YrA = self.YrA_full[:,self.miniscope_exp_fnum[self.start_idx]:self.miniscope_exp_fnum[self.stop_idx]]
        #self.S = self.S_full
        #self.C = self.C_full
        ...
        if self.YrA_full is not None:
            self.savefile_YrA_mov = os.path.join(self.savepath, self.session_type+'-YrA_mov-'+self.mouse+'.npy')
            self.savefile_YrA_imm = os.path.join(self.savepath, self.session_type+'-YrA_imm-'+self.mouse+'.npy')
        ...
        if self.behaviour_type == 'movement':
            ...
            if self.YrA_full is not None:
                if os.path.exists(self.savefile_YrA_mov) and os.path.exists(self.savefile_YrA_imm):
                    pass
                else:
                    self.YrA_mov = np.multiply(self.YrA, (self.velocities_miniscope_smooth[:self.YrA.shape[1]] >= 2.0).astype(int))
                    self.YrA_imm = np.multiply(self.YrA, (self.velocities_miniscope_smooth[:self.YrA.shape[1]] < 2.0).astype(int))
        ...
        #
        # Finally, filter out interneurons based firing rate histogram cutoff, with top 10% cells classified as interneurons.
        #
        '''
        duration = np.diff(self.miniscope_exp_fnum)[0] / MINISCOPE_FPS
        firing_rates = np.zeros(len(self.S_spikes))
        for i in range(len(firing_rates)):
            firing_rates[i] = len(self.S_spikes[i]) / duration
        self.pyr_frate_threshold = np.percentile(firing_rates, self.pyr_percentile_cutoff)
        self.pyr_mask = firing_rates < self.pyr_frate_threshold
    ...
    def get_A_matrix(self):
        ...
        if self.saver_A.check_exists('A_sparse') and self.saver_A.check_exists('A_idx'):
            A_data = self.saver_A.load('A_sparse')
            self.A = A_data['sparse'].toarray().reshape(A_data['shape']).astype(np.uint8)
            self.A_idx = self.saver_A.load('A_idx')
        elif self.saver_A.check_exists('A') and self.saver_A.check_exists('A_idx'):
            # Legacy dense format: load, convert, and re-save as sparse
            self.A = self.saver_A.load('A')
            self.A_idx = self.saver_A.load('A_idx')
            self._save_A_sparse(self.A)
            self.A = (self.A > 0).astype(np.uint8)
        else:
            self.set_minian_output_dir()
            ...
            self._save_A_sparse(self.A)
            self.saver_A.save(self.A_idx, 'A_idx')
            self.A = (self.A > 0).astype(np.uint8)

    def _save_A_sparse(self, A):
        """Save A matrix as binarized sparse CSR matrix."""
        shape = A.shape
        A_bin = (A > 0).astype(np.uint8).reshape(shape[0], -1)
        A_sparse = sp_sparse.csr_matrix(A_bin)
        self.saver_A.save({'sparse': A_sparse, 'shape': shape}, 'A_sparse')
...
for l in unit_id:
    ...
    def get_S_mapping(self, mapping, with_crossreg=None, want_peakval=False):
        '''
        ...
        '''
        if mapping == 'full':
            # All cells in this session — no crossreg needed.
            S_idx = self.S_idx
            indeces_into_S = list(range(len(S_idx)))
        else:
            if with_crossreg:
                crossreg = with_crossreg
            else:
    ...
    def process_binned_sp_rates_mapping(self, mapping, bin_width, plot_it=False, want_peakval=False, with_engram=False, crossreg=None, engram_cell_indices=None):
        '''
        ...
        Engram filtering: when ``with_engram=True``, ``engram_cell_indices``
        must be provided (an iterable of full-session S row indices that
        the unified engram pipeline classified as engram cells for this
        ...
        if with_engram:
            if engram_cell_indices is None:
                raise RuntimeError(
                    f"process_binned_sp_rates_mapping(with_engram=True) requires "
                    f"engram_cell_indices (mouse={self.mouse}, mapping={mapping})."
                )
            eng_set = set(int(i) for i in engram_cell_indices)
            keep_full_idx = [i for i in S_idx_full if int(i) in eng_set]
            # subset S, S_spikes, S_peakval to keep_full_idx (in original order)
            keep_pos = [S_idx_full.index(i) for i in keep_full_idx]
            S = S[keep_pos, :]
            S_spikes = {i: self.S_spikes[i] for i in keep_full_idx}
            if want_peakval:
                S_peakval = {i: self.S_peakval[i] for i in keep_full_idx}

        binned_rates = []
        t_length = S.shape[1]-1
        beg_period = range(0, t_length - bin_width, bin_width)
        end_period = range(bin_width, t_length, bin_width)
        if with_engram and len(S_spikes) == 0:
            # No engram cells survive the (mapping) intersection. Return
            # NaN per bin so callers can detect/skip rather than crashing.
            print(f"  [engram] {self.mouse} mapping={mapping}: 0 engram cells "
                  f"after mapping intersection -> returning NaNs", flush=True)
            n_bins = len(beg_period)
    ...
    def get_location_data(self):
        ...
        if self.behaviour_type == 'movement':
            ...
            if self.saver_loc_data.check_exists('loc_df'):
                ...
                self.set_minian_output_dir()
                num_files = len(glob.glob(os.path.join(self.behavcam_output_dir, "[0-9]_LocationOutput.csv")))
                loc_files = []
                ...
                if not loc_dfs:
                    print('  WARNING: No LocationOutput.csv files found for {} {} - disabling movement analysis'.format(self.mouse, self.session_type))
                    self.behaviour_type = None  # Can't do movement analysis without location data
                    return # don't have .csv files processed yet..

                loc_df = pd.concat(loc_dfs)                
                self.saver_loc_data.save(loc_df, 'loc_df')

            # Load centerline for LinearTrackSession
            if hasattr(self, 'saver_LT_centerline'):
                if self.saver_LT_centerline.check_exists('centerline'):
                    self.centerline = self.saver_LT_centerline.load('centerline')
                else:
                    # Check if centerline file exists in BehavCam directory and load it
                    centerline_path = os.path.join(self.behavcam_output_dir, 'LT_centerline.npy')
                    if os.path.exists(centerline_path):
                        self.centerline = np.load(centerline_path)
                        self.saver_LT_centerline.save(self.centerline, 'centerline')
                    else:
            ...
            print(f'  [LOC] {self.mouse} {self.session_type}: location data loaded OK '
                  f'({self.loc_X_miniscope_smooth.shape[0]} miniscope frames)')
...
class TraceFearCondSession(BehaviourSession):
    ...
    def find_exp_boundaries(self):
        ...
        # Add post-tone (just 20s before shock)
        for i in range(len(self.tone_offsets)):
            self.post_tone_onsets.append(self.tone_offsets[i])
            self.post_tone_offsets.append(self.shock_onsets[i])
...
class TestASession(BehaviourSession):
    """
    Context test in context A (the conditioning context). No tones or shocks are presented;
    the animal is simply placed in the context for ~5 minutes while freezing and neural 
    ...
    def find_period_bounds(self):
        '''
        Generate a list of tuples of onset,offset frames for behaviourally relevant periods.
        For Test A (context test), the entire session is a single 'context' period.
        '''
        period_bounds = []
        period_bounds.append(('context', (0, self.S.shape[1]-1)))
        self.period_bounds = period_bounds
...
class LinearTrackSession(BehaviourSession):
    ...
    def crop_XY(self):
        '''
        Re-anchor the behavcam and miniscope cams so that the origin is at (0,0), thus preventing the creation of too-large
        fluorescence maps.
        '''
        if hasattr(self, 'loc_X_behavcam_smooth_orig'):
            return
        
        self.loc_X_behavcam_smooth_orig = self.loc_X_behavcam_smooth
        self.loc_Y_behavcam_smooth_orig = self.loc_Y_behavcam_smooth
        # this returns a COPY so it's safe to pass like this
        self.loc_X_behavcam_smooth, self.loc_Y_behavcam_smooth, min_X, min_Y = reanchor_XY_to_zero(self.loc_X_behavcam_smooth, self.loc_Y_behavcam_smooth)
        self.centerline_behavcam = self.centerline - np.array([[min_X, min_Y]])

        self.loc_X_miniscope_smooth_orig = self.loc_X_miniscope_smooth
        self.loc_Y_miniscope_smooth_orig = self.loc_Y_miniscope_smooth
        self.loc_X_miniscope_smooth, self.loc_Y_miniscope_smooth, min_X, min_Y = reanchor_XY_to_zero(self.loc_X_miniscope_smooth, self.loc_Y_miniscope_smooth)
        self.centerline_miniscope = self.centerline - np.array([[min_X, min_Y]])

        self.loc_X_miniscope_orig = self.loc_X_miniscope
        self.loc_Y_miniscope_orig = self.loc_Y_miniscope
        self.loc_X_miniscope, self.loc_Y_miniscope, _, _ = reanchor_XY_to_zero(self.loc_X_miniscope, self.loc_Y_miniscope)

        self.loc_X_behavcam_orig = self.loc_X_behavcam
        self.loc_Y_behavcam_orig = self.loc_Y_behavcam
        self.loc_X_behavcam, self.loc_Y_behavcam, _, _ = reanchor_XY_to_zero(self.loc_X_behavcam, self.loc_Y_behavcam)

        for label, x_orig, y_orig, x_smooth, y_smooth in [
            ("Behavcam", self.loc_X_behavcam_orig, self.loc_Y_behavcam_orig, self.loc_X_behavcam_smooth, self.loc_Y_behavcam_smooth),
            ("Miniscope", self.loc_X_miniscope_orig, self.loc_Y_miniscope_orig, self.loc_X_miniscope_smooth, self.loc_Y_miniscope_smooth),
        ]:
            old_min_x = np.nanmin(x_orig)
            old_max_x = np.nanmax(x_orig)
            old_min_y = np.nanmin(y_orig)
            old_max_y = np.nanmax(y_orig)

            new_min_x = np.nanmin(x_smooth)
            new_max_x = np.nanmax(x_smooth)
            new_min_y = np.nanmin(y_smooth)
            new_max_y = np.nanmax(y_smooth)

            print(
            f"    Mouse {self.mouse} {label} re-anchoring "
            f"MIN_X: {old_min_x:.2f} -> {new_min_x:.2f}; "
            f"MIN_Y: {old_min_y:.2f} -> {new_min_y:.2f}; "
            f"MAX_X: {old_max_x:.2f} -> {new_max_x:.2f}; "
