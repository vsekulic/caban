"""caban.analyses — top-level analysis pipeline.

Contains every analysis block executed after data loading: spike-rate and
binned-rate panels, ROI / proportional-activity plots, PSTH, place fields,
LT pipeline (occupancy, PFs, decoding, zone cross-registration), 2D
Bayesian decoder paradigms A–F, TFC cross-session MixedLM, 2D PV
correlation, epoch-PV similarity, population PCA, Isomap, population
vectors / distances, binned-activity panels.

This module runs as a script in the notebook's globals. It references
bare names (``mouse_groups``, ``TFC_cond``, ``plot_PSTH``,
``DEVEL_SWITCH``, ``PLOTS_DIR``, …) that must already exist in the
calling globals. The expected invocation is from ``run_pipeline.ipynb``::

    ds = load_all_mice(cfg, ...)
    globals().update(vars(ds))             # expose loader outputs
    globals().update(cfg.export_globals()) # expose cfg switches as bare names
    exec(compile(open('caban/analyses.py').read(),
                 'caban/analyses.py', 'exec'), globals())

Each analysis block is gated by its own ``plot_*`` / ``enable_*`` /
``optimize_*`` switch from ``PipelineConfig``. Toggle switches on
``cfg`` and re-call ``globals().update(cfg.export_globals())`` to change
what runs.
"""
