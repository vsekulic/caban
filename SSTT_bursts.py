...
- SSTT_burst_detailed_statistics.txt: Detailed statistics report
...
DETAILED_STATS_FILE = OUTPUT_DIR / 'SSTT_burst_detailed_statistics.txt'
...
def run_statistical_analysis(filtered_df):
    ...
    for metric, title in BURST_METRICS.items():
        ...
        try:
            ...
            # Calculate 95% confidence interval for the coefficient
            ci = result.conf_int(alpha=0.05)['Group_Encoded']
            ci_lower = ci[0]
            ci_upper = ci[1]
    ...
    _write_detailed_stats_report(stats_results, filtered_df, exc_spikes, inh_spikes, ks_stat, ks_p_value)
...
def _write_detailed_stats_report(stats_results, filtered_df, exc_spikes, inh_spikes, ks_stat, ks_p_value):
    """
    Write a detailed statistics report to a text file.
    
    Args:
        stats_results: List of dictionaries with statistical results
        exc_spikes: numpy array of Exc group spike counts
        inh_spikes: numpy array of Inh group spike counts
        ks_stat: KS test statistic
        ks_p_value: KS test p-value
    """
    DETAILED_STATS_FILE.parent.mkdir(parents=True, exist_ok=True)
    
    with open(DETAILED_STATS_FILE, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("SST+ BURST ANALYSIS - DETAILED STATISTICAL REPORT\n")
        f.write("=" * 80 + "\n\n")
        ...
        f.write(f"Cell types included: {', '.join(CELL_TYPES_TO_INCLUDE)}\n")
        f.write(f"Groups compared: SST_hM3D (Exc) vs SST_hM4D (Inh)\n")
        f.write(f"Statistical method: Linear Mixed-Effects Models (LMM)\n")
        f.write(f"Random effect: (1 | Mouse)\n")
        f.write(f"Post-hoc test: Wald z-test (two-tailed)\n")
        f.write(f"Effect size: Cohen's d (pooled SD)\n")
        f.write(f"Multiple testing correction: Bonferroni (6 comparisons)\n")
        f.write(f"Spike count distribution test: Kolmogorov-Smirnov test\n\n")
        
        # LMM results for each metric
        f.write("=" * 80 + "\n")
        ...
        for result in stats_results:
            ...
            title = result['Natural_Title']
            
            f.write(f"Metric: {title} ({metric})\n")
            f.write("-" * 60 + "\n")
            
            if pd.notna(result['N_observations']):
                f.write(f"  Number of observations: {int(result['N_observations'])}\n")
            
            if pd.notna(result['Coefficient']):
                f.write(f"  LMM Coefficient (Exc - Inh): {result['Coefficient']:.4f}\n")
                f.write(f"  Standard Error: {result['SE']:.4f}\n")
                f.write(f"  Z-value: {result['Z_value']:.4f}\n")
                f.write(f"  95% Confidence Interval: [{result['CI_lower']:.4f}, {result['CI_upper']:.4f}]\n")
                f.write(f"  P-value (uncorrected): {result['P_value']:.6f}\n")
                f.write(f"  P-value (Bonferroni-corrected): {result['P_corrected']:.6f}\n")
                
                if pd.notna(result['Effect_Size_d']):
                    f.write(f"  Effect Size (Cohen's d): {result['Effect_Size_d']:.4f}\n")
                ...
                if pd.notna(p_corr):
                    if p_corr < 0.001:
                        ...
                        sig_text = "** p < 0.01 (significant)"
                    elif p_corr < 0.05:
                        sig_text = "* p < 0.05 (marginally significant)"
                    else:
                        sig_text = "ns p >= 0.05 (not significant)"
                    f.write(f"  Significance: {sig_text}\n")
            else:
                f.write("  [Insufficient data or model convergence error]\n")
        ...
        for metric, title in BURST_METRICS.items():
            ...
            exc_vals = filtered_df[filtered_df['Group_Label'] == 'Exc'][metric].dropna()
            inh_vals = filtered_df[filtered_df['Group_Label'] == 'Inh'][metric].dropna()
            
            if len(exc_vals) > 0:
                f.write(f"  Exc (SST_hM3D):\n")
                f.write(f"    N = {len(exc_vals)}\n")
                f.write(f"    Mean = {exc_vals.mean():.4f}\n")
                f.write(f"    Median = {exc_vals.median():.4f}\n")
                f.write(f"    SD = {exc_vals.std():.4f}\n")
                f.write(f"    Min = {exc_vals.min():.4f}\n")
                f.write(f"    Max = {exc_vals.max():.4f}\n")
            
            if len(inh_vals) > 0:
                f.write(f"  Inh (SST_hM4D):\n")
                f.write(f"    N = {len(inh_vals)}\n")
                f.write(f"    Mean = {inh_vals.mean():.4f}\n")
                f.write(f"    Median = {inh_vals.median():.4f}\n")
                f.write(f"    SD = {inh_vals.std():.4f}\n")
                f.write(f"    Min = {inh_vals.min():.4f}\n")
                f.write(f"    Max = {inh_vals.max():.4f}\n")
            
            f.write("\n")
        
        # Spike count distribution
        f.write("=" * 80 + "\n")
        f.write("SPIKE COUNT PER BURST DISTRIBUTION\n")
        f.write("=" * 80 + "\n\n")
        
        f.write(f"  Exc observations: {len(exc_spikes)}\n")
        f.write(f"  Inh observations: {len(inh_spikes)}\n")
        
        if len(exc_spikes) > 0 and len(inh_spikes) > 0:
            f.write(f"  KS statistic: {ks_stat:.4f}\n")
            f.write(f"  KS p-value: {ks_p_value:.6f}\n")
            
            if ks_p_value < 0.001:
                f.write("  Significance: *** p < 0.001 (highly significant)\n")
            elif ks_p_value < 0.01:
                f.write("  Significance: ** p < 0.01 (significant)\n")
            elif ks_p_value < 0.05:
                f.write("  Significance: * p < 0.05 (marginally significant)\n")
            else:
                f.write("  Significance: ns p >= 0.05 (not significant)\n")
        ...
        f.write(f"{'Metric':<30} {'Coeff':<10} {'SE':<10} {'Z':<10} {'P-corr':<12} {'d':<10} {'Sig':<6}\n")
        f.write("-" * 88 + "\n")
        
        for result in stats_results:
            metric_name = result['Natural_Title'][:28]
            coef = f"{result['Coefficient']:.4f}" if pd.notna(result['Coefficient']) else 'N/A'
            se = f"{result['SE']:.4f}" if pd.notna(result['SE']) else 'N/A'
            z = f"{result['Z_value']:.4f}" if pd.notna(result['Z_value']) else 'N/A'
            p_corr = f"{result['P_corrected']:.4g}" if pd.notna(result['P_corrected']) else 'N/A'
            d = f"{result['Effect_Size_d']:.3f}" if pd.notna(result['Effect_Size_d']) else 'N/A'
            sig = str(result['Significant']) if pd.notna(result.get('Significant', np.nan)) else 'N/A'
            f.write(f"{metric_name:<30} {coef:<10} {se:<10} {z:<10} {p_corr:<12} {d:<10} {sig:<6}\n")
        
        f.write("\n" + "=" * 80 + "\n")
        f.write("END OF REPORT\n")
        f.write("=" * 80 + "\n")
    
    print(f"  Detailed statistics report saved to: {DETAILED_STATS_FILE}")
...
def main():
    ...
    print(f"  Detailed statistics report: {DETAILED_STATS_FILE}")
